...
console.log(color(figlet.textSync('Queen Bot MD', {
		font: 'Pagga',
...
console.log(color('\nYou can follow me on GitHub: DarkWinzo','aqua'))
    ...
    const Queen = QueenConnect({
        ...
        browser: ['Queen-MD By : DarkWinzo','Safari','1.0.0'],
        auth: state,
        version
    })
...
console.log(err)
    ...
    Queen.ev.on('groups.update', async pea => {
       ...
       Queen.sendMessage(m.chat, { image: wm_fatih, caption: 'Grop has been *Opened!* Now *Everyone* can send Messages!'})
        ...
        Queentextddfq =`Group Subject යාවත්කාලීන කර ඇත To:\n\n*${pea[0].subject}*`
        ...
        try {
            ...
            for (let num of participants) {
                ...
                try {
                    ...
                    ppgroup = 'https://i.ibb.co/jgbdv7F/Queen-Bot-copy.jpgg'
                }

                let targetname = await Queen.getName(num)
                grpmembernum = metadata.participants.length

                QueenWelcomepp = await getBuffer(`https://hardianto.xyz/api/welcome3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(targetname)}&bg=https://telegra.ph/file/d460e086f9f9bf6b04e17.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(grpmembernum)}`)
                QueenByepp = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(targetname)}&bg=https://telegra.ph/file/d460e086f9f9bf6b04e17.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(grpmembernum)}`)
...
I am *Queen*, Welcome to ${metadata.subject}.
...
let Queenbuttons = [
    {buttonId: `none`, buttonText: {displayText: '🏵️ සාදරයෙන් පිළිගනිමු යාලුවනේ 🏵️'}, type: 1}
    ]
    let buttonMessage = {
...
let Queenbuttons = [
    {buttonId: `none`, buttonText: {displayText: '👋 බායි යාලුවනේ 👋'}, type: 1}
    ]
    let buttonMessage = {
    jpegThumbnail:ppuser,
    mentions: [num],
