...
console.log(color('\nHello, I am DarkWinzo,The Main Developer of Queen-MD Bot.\n\nThanks for Using Queen-MD Bot','aqua'))
console.log(color('\nYou can Follow me on GitHub: DarkWinzo','aqua'))
    ...
    const Queen = QueenConnect({
        ...
        browser: ['QUEEN-MD: DARKWINZO','Safari','1.0.0'],
        auth: state,
        version
    })
...
Queen.ev.on('groups.update', async pea => {
       ...
       ppgc = 'https://i.ibb.co/jgbdv7F/Queen-Bot-copy.jpg'
       }
...
*/
    ...
    Queen.ev.on('groups.update', async pea => {
        ...
        ppgc = 'https://i.ibb.co/n3LnXsy/Queen.png'
        }
        let wm_fatih = { url : ppgc }
        if (pea[0].announce == true) {
...
`
    ...
    Queen.ev.on('connection.update', async (update) => {
        ...
        let reason = lastDisconnect.error ? lastDisconnect?.error?.output.statusCode : 0;
            if (reason === DisconnectReason.badSession) { console.log(`වැරදි session ගොනුවක්, කරුණාකර එය මකා දමා නැවත ස්කෑන් කරන්න`); process.exit(); }
            else if (reason === DisconnectReason.connectionClosed) { console.log("සම්බන්ධතාවය වසා ඇත, නැවත සම්බන්ධ වෙමින්...."); startQueen(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("සේවාදායකයෙන් සම්බන්ධතාව නැති විය, නැවත සම්බන්ධ වෙමින්..."); startQueen(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("සම්බන්ධතාවය ප්‍රතිස්ථාපනය කරන ලදී, තවත් නව session ගොනුවක් විවෘත කරන ලදී, කරුණාකර වත්මන් session ගොනුව පළමුව වසන්න"); process.exit(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`උපාංගය logout විය, කරුණාකර session ගොනුව මකා දමා නැවත ස්කෑන් කරන්න.`); process.exit(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("නැවත ආරම්භ කිරීමට අවශ්‍යයි, නැවත ආරම්භ කිරීම..."); startQueen(); }
            else if (reason === DisconnectReason.timedOut) { console.log("සම්බන්ධතා කාලය අවසන්, නැවත සම්බන්ධ වෙමින්..."); startQueen(); }
