...
Queen.sendMessage(m.chat, {text: teks, mentions:[m.sender]}, {quoted:m})
                                ...
                                console.error("❌ කිසියම් දෝෂයක් සිදු වී ඇත. !")
                        }
                }
                if (prefix && command) {
                                const currentLevel = getLevelingLevel(m.sender)
...
reply(`
කරුණාකර ඔහුව ටැග් කිරීමට උත්සාහ නොකරන්න
ඔහු නොබැඳිය.(Offline) ${user.afkReason ? ' after ' + user.afkReason : ''}
...
case 'me': case 'profile': case 'p':
  ...
  if (!isInventoriBuruan){ addInventoriBuruan(m.sender) }
     ...
     try {
      ...
      pfp ='https://i.ibb.co/jgbdv7F/Queen-Bot-copy.jpg'
    }
...
let mentioned = participants.map(v => v.jid)
    ...
    case 'nowa':  case 'stalk': case 'stalknumber':{
        ...
        if (!inputnumber.includes('x')) return reply('ඔබ එකතු කර නැත x')
...
*Select video or audio and wait a while*`
 ...
 case 'ytvd': {
    if (isBan) return reply(mess.banned)	 			
 if (isBanChat) return reply(mess.bangc)
 Queen.sendMessage(from, {video:{url:args[0]}, mimetype:"video/mp4", caption:"මේ තියෙන්නේ...",}, {quoted:m})
 }
 break
...
if (isBanChat) return reply(mess.bangc)
         ...
         Queen.sendMessage(m.chat, { image: { url: random.male }, caption: `ඔහු වෙනුවෙන්...` }, { quoted: m })
         Queen.sendMessage(m.chat, { image: { url: random.female }, caption: `ඇය වෙනුවෙන්...` }, { quoted: m })
     }
 break
...
if (!text) return reply(`උදාහරණයක් ලෙස : ${prefix + command} https://youtu.be/0VVfDGdDJ0o 128kbps`)
if (!isUrl(args[0]) && !args[0].includes('youtube.com')) return reply(`ඔබ ලබා දුන් සබැඳිය වලංගු නැත`)
...
if (!text) return reply(`උදාහරණයක් ලෙස : ${prefix + command} https://youtu.be/0VVfDGdDJ0o 360p`)
if (!isUrl(args[0]) && !args[0].includes('youtube.com')) return reply(`ඔබ ලබා දුන් සබැඳිය වලංගු නැත!`)
...
case 'pinterest': case 'pin': {
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
 if (!args.join(" ")) return reply("කරුණාකර සෙවුම් පදයක් සපයන්න!")
 try {
...
case 'swm': case 'take': case 'stickerwm': case 'steal':{
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
if (!args.join(" ")) return reply(`මෙම විධානය භාවිතා කරන්න: -steal Queen|By: DarkWinzo`)
const swn = args.join(" ")
...
if ((quoted.msg || quoted).seconds > 11) return reply('උපරිම තත්පර 10කට අවසර ඇත!')
let media = await quoted.download()
...
return reply('දෝෂයක් හටගැනිණි.ප්‍රතිඵලය හමු නොවීය!') 
}) 
...
let buttons = [
{ buttonId: '❤️', buttonText: { displayText: 'සුභ පැතුම් ❤️' }, type: 1 }
]
...
let ments = [me, jodoh]
let buttons = [
{ buttonId: '❤️', buttonText: { displayText: 'මගේ ආත්ම සහකරු/සහකාරිය වන්න ❤️' }, type: 1 }
]
...
case 'charactercheck':
    ...
    if (isBanChat) return reply(mess.bangc)
					if (!text) return replay(`කවුරුහරි ටැග් කරන්න, උදාහරණයක් ලෙස : ${prefix + command} @DarkWinzo`)
					const Queentttt =['Compassionate','Generous','Grumpy','Forgiving','Obedient','Good','Simp','Kind-Hearted','patient','UwU','top, anyway','Helpful']
					const taky = Queentttt[Math.floor(Math.random() * Queentttt.length)]
					Queen.sendMessage(from, { text: `Character Check : ${q}\nAnswer : *${taky}*` }, { quoted: m })
				     break
    ...
    if (isBanChat) return reply(mess.bangc)
         ...
         "breakup with your best friend for 5hrs without telling him/her that its a dare",
          ...
          "send abusive words in any grup, excepting this grup, and send screenshot proof here"
                   ...
                   buffer = await getBuffer(`https://i.ibb.co/jgbdv7F/Queen-Bot-copy.jpg`)
                   Queen.sendMessage(from, { image: buffer, caption: '*ඔබ ඩෙයා එක තෝරාගෙන ඇත*\n\n'+ Queendareww }, {quoted:m})
                   break
...
case 'truth':
    ...
    if (isBanChat) return reply(mess.bangc)
                 ...
                 "do you play pubg, if you then send ur id number"
                           ...
                           buffer = await getBuffer(`https://i.ibb.co/jgbdv7F/Queen-Bot-copy.jpg`)
                           Queen.sendMessage(from, { image: buffer, caption: '*You have chosen Truth*\n'+ Queentruthww }, {quoted:m})
                           break
...
case 'crossplay': case 'crosplay': case 'cosplay':
    ...
    if (!m.isGroup) return replay(mess.grouponly)
        ...
        caption: "මම කවුද කියලා අනුමාන කරන්න...",
        footer: `${global.BotName}`,
        buttons: buttons,
        headerType: 4
        }
...
if(users == "none"){
     musers =`@${m.sender.split("@")[0]} ${command}ed තමන් සමඟ!`
     console.log(musers)

} else {
const rcpp =`@${users.split("@"[0])}`
 musers= `@${m.sender.split("@")[0]} ${command}ed සමඟ @${users.split("@")[0]} `
...
if(users == "none"){
     musers =`@${m.sender.split("@")[0]} තමන් එක්කම කනවා!`
     console.log(musers)

} else {
const rcpp =`@${users.split("@"[0])}`
 musers= `@${m.sender.split("@")[0]} සමඟ  @${users.split("@")[0]} ආහාර ගනිමින් සිටී`
...
if(users == "none"){
     musers =`@${m.sender.split("@")[0]} තමන්වම වැළඳ ගත්තා!`
     console.log(musers)

} else {
const rcpp =`@${users.split("@"[0])}`
 musers= `@${m.sender.split("@")[0]} වැළඳ ගත්තා @${users.split("@")[0]} `
...
if(users == "none"){
     musers =`@${m.sender.split("@")[0]} තනියම නටනවා!!`
     console.log(musers)

} else {
const rcpp =`@${users.split("@"[0])}`
 musers= `@${m.sender.split("@")[0]}  නටනවා @${users.split("@")[0]} එක්ක`
...
if(users == "none"){
     musers =`@${m.sender.split("@")[0]} ${command}ed තමන්ම !`
     console.log(musers)
...
if(users == "none"){
     musers =`@${m.sender.split("@")[0]} ${command}ed තමන් තුළම!`
     console.log(musers)

} else {
const rcpp =`@${users.split("@"[0])}`
 musers= `@${m.sender.split("@")[0]} ${command}ed තුළම @${users.split("@")[0]} `
...
case 'animewall2': case 'animewallpaper2':
    ...
    reply(mess.waiting)						
const { AnimeWallpaper } =require("anime-wallpaper")
if(!q) return reply('කරුණාකර සෙවුම් පදයක් සපයන්න!')
const wall = new AnimeWallpaper();
    const pages = [1,2,3,4];
        const random=pages[Math.floor(Math.random() * pages.length)]
...
case 'anime':
    ...
    if (isBanChat) return reply(mess.bangc)
    if (!m.isGroup) return replay(mess.grouponly)
	    if(!q) return reply(`කරුණාකර සෙවුම් පදයක් සපයන්න!\n\n*උදාහරණයක් ලෙස:* ${prefix}anime Queen`)
reply(mess.waiting)							
...
if(!q) return reply(`කරුණාකර සෙවුම් පදයක් සපයන්න!\n\n_උදාහරණයක් ලෙස:_ ${prefix}manga Queen`)
...
reply(mess.waiting)	
 ...
 var wbuttsssr = [
    {buttonId: `-waifu`, buttonText: {displayText: `>>`}, type: 1},
        ...
        caption: ' waifu එකකට වඩා නිසැකවම ඔබේ ජීවිතය විනාශ කරනු ඇත!',
        buttons: wbuttsssr,
        headerType: 4
         }     
...
case 'bc': case 'broadcast': case 'bcall': {
    ...
    if (isBanChat) return reply(mess.bangc)
if (!isCreator) return replay(mess.botowner)
if (!args.join(" ")) return replay(`කරුණාකර broadcast කිරීමට පෙළ කිහිපයක් ඇතුළත් කරන්න ! \n\nඋදාහරණයක් ලෙස : ${prefix + command} ${global.OwnerName}`)
let anu = await store.chats.all().map(v => v.id)
replay(`Broadcast යවන්න  To ${anu.length} Chat\nTime's up ${anu.length * 1.5} second`)
for (let yoi of anu) {
await sleep(1500)
let btn = [{
quickReplyButton: {
...
case '':
    ...
    if (isBanChat) return reply(mess.bangc)
      ...
      Queenpic ='https://i.ibb.co/3mL4HwL/Queen-MD.jpg'
